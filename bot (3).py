...
def fetch_article_image(url: str) -> str:
    """Fetch the article page and return its og:image URL, or empty string."""
    try:
        ...
        log.warning("Could not fetch article image: %s", e)
    return ""
...
def scrape_latest() -> list[dict]:
    ...
    for a_tag in soup.find_all("a", href=True):
        ...
        break  # only need the top article

    if articles:
        articles[0]["image"] = fetch_article_image(articles[0]["url"])
...
async def check_news():
    ...
    if aid in seen_ids:
        log.info("Top article unchanged: %s", aid)
        return
    ...
    try:
        ...
        seen_ids.add(aid)
        save_seen(seen_ids)
        log.info("Posted: %s", article["title"])
